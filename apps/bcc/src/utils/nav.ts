...
/**
 * Builds the breadcrumb trail for a pathname. Dashboard is always the root; a
 * matched nav item supplies the label, otherwise it is derived from the path.
 */
export function buildBreadcrumbs(pathname: string): Crumb[] {
  const crumbs: Crumb[] = [
    { label: 'Dashboard', path: ROUTES.dashboard, isCurrent: pathname === ROUTES.dashboard },
  ];
