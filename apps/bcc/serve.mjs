// Production static file server for the built Business Control Center (apps/bcc/dist).
//
...
//
// Usage:  node serve.mjs        (from apps/bcc, via `pnpm start`)
...
if (!existsSync(DIST_DIR)) {
  console.error(
    `[serve] Build output not found at ${DIST_DIR}. Run "pnpm --filter @superdreams/bcc build" first.`,
  );
  process.exit(1);
}
...
server.listen(PORT, HOST, () => {
  console.log(`[serve] Business Control Center serving ${DIST_DIR} on http://${HOST}:${PORT}`);
});
