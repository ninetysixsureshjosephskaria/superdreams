 ...
 *
 * Creates N fully-populated member accounts and prints a credentials table. The
 * generation logic lives in `bulk-members-core.ts` (side-effect free) and is
 * shared with the Super Admin "Generate Demo Members" BCC action.
 *
...
async function main(): Promise<void> {
  ...
  try {
    const { created, skipped } = await seedBulkMembers(connection.db, COUNT);
    ...
    process.stdout.write(`\nCreated ${created.length}, skipped ${skipped} (already existed).\n`);
  } finally {
